...
import EventImpact from "./EventEmpact";
  ...
  const fetchAnalysis = async () => {
    try {
      const analysisResponse = await axios.get(
        "http://localhost:5000/api/analysis-metrics"
      );
  ...
  const fetchAdditionalData = async () => {
    try {
      const yearlyResponse = await axios.get(
        "http://localhost:5000/api/average-yearly-price"
      );
      const distributionResponse = await axios.get(
        "http://localhost:5000/api/price-distribution"
      );
      ...
      <Row className="mb-4">
        <Col>
          ...
          <p className="dashboard-description">
            ...
            Use the filters below to narrow down the data, view analysis
